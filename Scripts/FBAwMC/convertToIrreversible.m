...
%% Loop through each column/rxn in the S matrix building the irreversible model
cnt = 0;
for i = 1:nRxns
    cnt = cnt + 1;
    
    % Expand the new model (same for both irreversible and reversible reactions
    modelIrrev.rev(cnt) = model.rev(i);
    irrev2rev(cnt) = i;

    % Check if reaction is declared as irreversible, 
    % but bounds suggest reversible (i.e., having both positive and negative bounds
